...
async function upload({ host, sshKey }, from, to){
  const dir = to.split('/')
  dir.pop()
  await runRemotely(servers['i2_dev_api'], `mkdir -p ${dir.join('/')}`)
...
async function autoReleaseDockerWithNotify(config, notify) {
  const imageTag = await getCommitHash()
...
const deployScalableImage = async (config, server, cmd, testUrl) => {

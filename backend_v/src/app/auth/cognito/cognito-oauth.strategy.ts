  ...
  constructor(
    ...
    super({
      authorizationURL: CognitoOauthStrategy.authorizationUrl(
        configService.get<string>('auth.oauth.cognito.domain'),
        configService.get<string>('auth.oauth.cognito.region'),
      ),
      tokenURL: CognitoOauthStrategy.tokenUrl(
        configService.get<string>('auth.oauth.cognito.domain'),
        configService.get<string>('auth.oauth.cognito.region'),
      ),
      clientID: configService.get<string>('auth.oauth.cognito.id'),
      clientSecret: configService.get<string>('auth.oauth.cognito.secret'),
      callbackURL: configService.get<string>('auth.oauth.cognito.redirectUrl'),
    });
    this.domain = configService.get<string>('auth.oauth.cognito.domain');
    this.region = configService.get<string>('auth.oauth.cognito.region');
  }

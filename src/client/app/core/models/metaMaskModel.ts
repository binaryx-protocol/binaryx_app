...
type MetaMaskState = {
  ...
  errors: ('ACCOUNT_ACCESS_REJECTED' | 'CHAIN_INVALID')[]
}
...
export const $walletConnect = atom(
  ...
  (get, set) => {
    ...
    const main = async () => {
      ...
      console.log('ACC1', get($metaMaskState)?.accounts?.[0])
      ...
      console.log('ACC2', get($metaMaskState)?.accounts?.[0])

      // await waitFor(() => !!get($metaMaskState)?.accounts?.[0])

      const isAuthorized = !!get($metaMaskState)?.accounts?.[0]
      if (!isAuthorized) {
        await ethereum.request({method: 'eth_requestAccounts'})
          ...
          .catch((err: any) => {
            if (err.code === 4001) {
              update({
                errors: ['ACCOUNT_ACCESS_REJECTED']
              })
        ...
        console.log('ACC3', get($metaMaskState)?.accounts?.[0])
      }

      await ethereum.request({ method: 'eth_chainId' })
        .then(onChainIdChange)

      console.log('CHAIN', get($metaMaskState)?.chainId)
      ...
      console.log('CHAIN2', get($metaMaskState)?.chainId)

      await waitFor(() => !!get($metaMaskState)?.chainId)

      if (get($metaMaskState)?.chainId !== get($rpcConfig)?.chain.chainId) {
        update({
          errors: ['CHAIN_INVALID']
        })
      }
